                        ...
                        <form method="POST" action="{{route('product.store')}}" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input name="model" class="form-control @error('name') is-invalid @enderror" value="{{ old('model') }}" type="text" placeholder="Enter Model">
                                    @error('model')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    {{-- <select name="unit_id" class="form-control">
                                        ...
                                        @foreach($units as $unit)
                                            <option value="{{$unit->id}}" {{ old($unit->id) == $unit->id ? 'selected' : '' }}>{{$unit->name}}</option>
                                        @endforeach
                                    </select> --}}
                                    <select name="unit_id" class="form-control">
                                        <option value="">---Select Unit---</option>
                                        @foreach($units as $unit)
                                            <option value="{{ $unit->id }}" {{ old('unit_id', $unit->unit_id) == $unit->id ? 'selected' : '' }}>
                                                {{ $unit->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select name="tax_id" class="form-control">
                                        ...
                                        @foreach($taxes as $tax)
                                            <option value="{{ $tax->id }}" {{ old('tax_id', $tax->id) == $tax->id ? 'selected' : '' }}>{{$tax->name}} %</option>
                                        @endforeach
                                    </select>
                                    @error('tax_id')
                                    <span class="invalid-feedback" role="alert">
                                        ...
                                        <div class="form-group col-md-5">
                                             <select name="supplier_id[]" class="form-control">
                                                ...
                                                @foreach($suppliers as $supplier)
                                                    <option value="{{$supplier->id}} ?? {{ old($supplier->id) }}">{{$supplier->name}} </option>
                                                @endforeach
                                            </select>
                                         </div>
                                        <div class="form-group col-md-5">
